...
environment by setting a `password` environment variable under `env` in
values.yaml. This should be a secretKeyRef to a secret key containing your
desired password. This will create a `kong_admin` admin whose token and
basic-auth password match the value in the secret. For example:
